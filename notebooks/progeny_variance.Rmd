...
* **Median number of passing regions per sire is `rsire_pair %>% select(sire_reg, V1, V2) %>% tidyr::pivot_longer(cols = c("V1", "V2")) %>% select(-name) %>% distinct() %>% group_by(sire_reg) %>% summarise(n = n_distinct(value)) %>% ungroup() %>% summarise(median(n)) %>% pull(`median(n)`)`**
