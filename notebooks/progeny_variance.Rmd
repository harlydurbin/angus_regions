...
baseline_change %>% 
  ...
  summarise(spread = max(median_v1)-min(median_v1)) %>% 
  ungroup() %>% 
  filter(spread >= 18.1437+(12.30019*3)) %>% 
  arrange(desc(spread))
```
...
Sires with at least 50 calves and at least two unique contemporary groups in two or more regions were retained for analysis (n = `r scales::comma(length(unique(baseline_change$sire_reg)))`).

Of sires with at least 50 calves in multiple regions,`r baseline_change %>% filter(change < -50) %>% summarise(n_distinct(sire_reg)) %>% pull(1) %>% scales::comma()` had at least one region in which the median weaning weight of calves in that region was at least 50 pounds lower than the median weaning weight of their calves across all regions. Most often this region was the Fescue Belt, occurring > 60% more often than the second ranked region (the Southeast). The median inter-region spread of weaning weights was about 45 pounds, though 5 sires had an inter-region spread greater than or equal to 200 lbs. 
