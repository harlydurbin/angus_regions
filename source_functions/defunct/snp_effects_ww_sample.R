...
start %>%
  ...
  arrange(`3`) %>%
  select(format_reg, cg_new, `3`, everything()) %>%
  write_delim(here::here(
    glue(
      "data/derived_data/snp_effects_ww/3v{other_region}/data.txt"
    )
  ),
  delim = " ",
  col_names = FALSE)
...
write_delim(start_ped, here::here(
  glue(
    "data/derived_data/snp_effects_ww/3v{other_region}/ped.txt"
  )
),
delim = " ",
col_names = FALSE)
...
genotyped_all %>%
  ...
  write_delim(here::here(
    glue(
      "data/derived_data/snp_effects_ww/3v{other_region}/pull_genotypes.txt"
    )
  ),
  delim = " ",
  col_names = FALSE)
...
start %>%
  ...
  write_delim(here::here(
    glue(
      "data/derived_data/snp_effects_ww/3v{other_region}/sample_stats.txt"
    )
  ),
  delim = " ")
