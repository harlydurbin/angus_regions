...
## --------------------------------------------------------------
c(1, 2, 5, 7, 8, 9) %>%
  purrr::map( ~ write_tworegion_data(
    iter = iter,
    comparison_region = .x,
    df = iter_data
  ))
...
iter_data %>%
  ...
  dplyr::select(format_reg, cg_new, `3`, `1`, `2`, `5`, `7`, `8`, `9`) %>%
  write_delim(here::here(
    glue(
      "data/derived_data/varcomp_ww/iter{iter}/all/data.txt"
    )
  ),
  delim = " ",
  col_names = FALSE)
...
c(1, 2, 5, 7, 8, 9) %>%
  purrr::map(
    ~ iter_genotyped %>%
      ...
      write_delim(here::here(
        glue(
          "data/derived_data/varcomp_ww/iter{iter}/3v{.x}/pull_list.txt"
        )
      ),
      delim = " ",
      col_names = FALSE)
...
iter_genotyped %>%
  ...
  write_delim(here::here(
    glue(
      "data/derived_data/varcomp_ww/iter{iter}/all/pull_list.txt"
    )
  ),
  delim = " ",
  col_names = FALSE)
